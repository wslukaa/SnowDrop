...
$strsql = "SELECT timestamp FROM insect";
$result = mysql_db_query($mysql_database, $strsql, $conn);

mysql_data_seek($result, mysql_num_rows($result)-1);
$current_time = mysql_fetch_row($result)[0];

$str_pm_data = "SELECT DISTINCT name,pm25 FROM insect WHERE timestamp='".$current_time."' and area='".$city."'";
...
while($row = mysql_fetch_array($result_pm_data))
{
	...
	$unit_data['pm'] = $row[1];
	$lresult = mysql_db_query($mysql_database, "SELECT lat,lng FROM station WHERE name = '".$row[0]."'", $conn);
