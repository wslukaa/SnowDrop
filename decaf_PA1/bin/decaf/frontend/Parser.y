...
%token SELF_ADD AELF_REDUCE
%token TERNARY_FIRST TERNARY_SECOND
%token NUMINSTANCES
%token FI SEPARATOR
%token DO OD
%token '+'  '-'  '*'  '/'  '%'  '='  '>'  '<'  '.'
...
%right TERNARY_FIRST TERNARY_SECOND
%left OR
%left AND 
...
Expr            :	LValue
                ...
                |	Expr TERNARY_FIRST Expr TERNARY_SECOND Expr
                	{
                		$$.expr = new Tree.Ternary(Tree.COND, $1.expr, $3.expr, $5.expr, $2.loc, $4.loc);
                	}
...
GuardedIfStmt		:	IF GuardedStmts FI
					{
						$$.stmt = new GuardedIf($2.slist, $2.elist, $1.loc);
					}
				;
				
GuardedDoStmt		:	DO GuardedStmts OD
					{
						$$.stmt = new GuardedDo($2.slist, $2.elist, $1.loc);
					}
				;
